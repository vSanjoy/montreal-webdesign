...
$(document).ready(function() {
	...
	$(document).on('click', '.click-to-open', function() {
		...
		if (id != '') {
			$('.preloader').show();
			var getPopupDataUrl = $('#admin_url').val()+'/quote/view/'+id;
			$.ajax({
				headers: {
					'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
				},
				...
				success: function (response) {
					...
					if (response.type == 'success') {
						$('#quote-data').html(response.message);
					} else {
						$('#quote-data').html('<tr><td colspan="2">{{trans("custom_admin.message_no_records_found")}}</td></tr>');
					}
				}
			});
		} else {
