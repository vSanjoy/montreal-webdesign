					...
					{{ Form::open([
						...
						<div class="form-body mt-4-5">
							...
							<div class="row mt-1">
								<div class="col-md-12">
									<div class="form-group">
										<label class="text-dark font-bold">@lang('custom_admin.label_trip_categories')<span class="red_star">*</span></label>
										<select class="form-control select2" id="category" name="category[]" multiple="multiple">
									@if (count($categoryList) > 0)
										@foreach ($categoryList as $category)
											<option value="{{$category->id}}" @if(in_array($category->id,$selectedCategories) || in_array($category->id, $categoryIds))selected @endif>{{$category->title}}</option>
										@endforeach
									@endif                                
										</select>
									</div>
							...
							@php
							$image = '';
							if ($details->logo != null && file_exists(public_path('images/uploads/'.$pageRoute.'/'.$details->logo))) {
								$image = '<img src="'.asset("images/uploads/vendor/".$details->logo).'" />';
							}
							@endphp
							<div class="row mt-1">
								<div class="col-md-12">
										...
										<div class="col-md-2">
											...
											@if ($details->logo != null)
												@if (file_exists(public_path('/images/uploads/'.$pageRoute.'/'.$details->logo)))
													<div class="image-preview-holder" id="logo_holder_image">
														<a data-fancybox="gallery" href="{{ asset('images/uploads/'.$pageRoute.'/'.$details->logo) }}">
															<img class="image-preview-border" id="logo_preview mt-2" src="{{ asset('images/uploads/'.$pageRoute.'/'.$details->logo) }}" width="90" />
														</a>
